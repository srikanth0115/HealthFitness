...
public class BleHeartRateSensor extends BleSensor<float[]> {
   ...
   /* private static final int SENSOR_BODY_LOCATION_OTHER = 0;
    ...
    public boolean onCharacteristicRead(BluetoothGattCharacteristic c) {
        ...
        Utility.showLog(TAG, "onCharacteristicsReas");

        if (!c.getUuid().toString().equals(UUID_SENSOR_BODY_LOCATION))
            return false;

        location = c.getProperties();
        Utility.showLog(TAG, "Sensor body location: " + location);
        return true;
    }
    ...
    private static double extractHeartRate(
        ...
        Utility.showLog(TAG, "Heart rate flag: " + flag);
        int format = -1;
        // Heart rate bit number format
        if ((flag & 0x01) != 0) {
            format = BluetoothGattCharacteristic.FORMAT_UINT16;
            Utility.showLog(TAG, "Heart rate format UINT16.");
        } else {
            format = BluetoothGattCharacteristic.FORMAT_UINT8;
            Utility.showLog(TAG, "Heart rate format UINT8.");
        }
        final int heartRate = characteristic.getIntValue(format, 1);
        Utility.showLog(TAG, String.format("Received heart rate: %d", heartRate));
        return heartRate;
    }
    ...
    private static double extractContact(
        ...
        if ((flag & 0x02) != 0) {
            Utility.showLog(TAG, "Heart rate sensor contact info exists");
            if ((flag & 0x04) != 0) {
                Utility.showLog(TAG, "Heart rate sensor contact is ON");
            } else {
                Utility.showLog(TAG, "Heart rate sensor contact is OFF");
            }
        } else {
            Utility.showLog(TAG, "Heart rate sensor contact info doesn't exists");
        }
        //final int heartRate = characteristic.getIntValue(format, 1);
        //Utility.showLog(TAG, String.format("Received heart rate: %d", heartRate));
        return 0.0d;
    ...
    private static double extractEnergyExpended(
        ...
        if ((flag & 0x08) != 0) {
            Utility.showLog(TAG, "Heart rate energy calculation exists.");
        } else {
            Utility.showLog(TAG, "Heart rate energy calculation doesn't exists.");
        }
        //final int heartRate = characteristic.getIntValue(format, 1);
        //Utility.showLog(TAG, String.format("Received heart rate: %d", heartRate));
        return 0.0d;
    ...
    private static Integer[] extractBeatToBeatInterval(
        ...
        if ((flag & 0x08) != 0) {
            ...
            offset += 2;
            Utility.showLog(TAG, "Received energy: {}" + energy);
        }
        if ((flag & 0x16) != 0) {
            // RR stuff.
            Utility.showLog(TAG, "RR stuff found at offset: " + offset);
            Utility.showLog(TAG, "RR length: " + (characteristic.getValue()).length);
            rr_count = ((characteristic.getValue()).length - offset) / 2;
            Utility.showLog(TAG, "RR length: " + (characteristic.getValue()).length);
            Utility.showLog(TAG, "rr_count: " + rr_count);
            if (rr_count > 0) {
                ...
                for (int i = 0; i < rr_count; i++) {
                    ...
                    offset += 2;
                    Utility.showLog(TAG, "Received RR: " + mRr_values[i]);
                }
                return mRr_values;
            }
        }
        Utility.showLog(TAG, "No RR data on this update: ");
        return null;
    }
